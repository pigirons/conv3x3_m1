...
int main(int argc, char *argv[])
{
    ...
    int buf_size =
        conv_winograd_b2f3s1_buf_size_m1(src_h, src_w, in_c, pad_h, pad_w, out_c);
    int cvt_filter_size =
        conv_winograd_b2f3s1_cvt_filter_size_m1(in_c, out_c);
    ...
    conv_winograd_b2f3s1_cvt_filter_m1(filter, in_c, out_c, buffer, cvt_filter);

    // warm up
    for (i = 0; i < loop_time; i++)
    {
        conv_winograd_b2f3s1_m1(src, src_h, src_w, in_c,
